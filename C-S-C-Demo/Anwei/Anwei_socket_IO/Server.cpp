...
void OnSelect(SOCKET ServerSocket)
{
	while (true) {
		// Select Model:
		// the frist Create and Init set.
		fd_set readfds; // Create read-Socket
		fd_set writefds;// Create write-Socket [此处暂时不会用到]
		fd_set exceptfds;// Create exception-Socket [error socket set 此处暂时不会用到]
		...
		int ret = (int)select(ServerSocket + 1, &readfds, &writefds, &exceptfds, &tv);
		if (ret < 0) // Select work done. No Work,No client - no client in read-set/write-set/exception-set and g_clientList is empty.
		{
			std::cout << "Select done. terminal code: " << WSAGetLastError() << std::endl;
			break;
		}


		for (int k = 0; k < (int)readfds.fd_count; ++k) {
			if (-1 == MsgProc(readfds.fd_array[k]))
			{
...
int main(int argc, char* args[])
{
	...
	std::thread t3(&OnSelect, ServerSocket);
	t3.detach();
