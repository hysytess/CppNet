...
//  Read byte stream
	...
	bool Read(T& n,bool bOffset = true)
	{
		...
		//方法1 断言assert()
		//2 CellLog
		//CellLog_Debug("error,CellStream::Read failed.");
		return false;
	}
	...
	uint32_t ReadArray(T* pArr, uint32_t len)
	{
		...
		//CellLog_Debug("error, CELLStream::ReadArray failed.");
		return 0;
	}
...
//  Write byte stream
	...
	bool Write(T n)
	{
		...
		//CellLog_Debug("error,CellStream::Write failed.");
		return false;
	}
	template<typename T>
	bool WriteArray(T* pData, uint32_t len)
	{
		...
		//CellLog_Debug("error,CellStream::WriteArray failed.");
		return false;
	}
